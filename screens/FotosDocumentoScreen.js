...
export default function FotosDocumentoScreen({ navigation, route }) {
    ...
    try {
      for (const photoUri of photos) {
        ...
        await database.execAsync(
          ...
          [registroId, base64, 'documento']
        );
      }
      
      if (photos.length > 0) {
        Alert.alert('Sucesso', 'Fotos do documento salvas!');
      }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomButtons}>
        <TouchableOpacity style={styles.saveButton} onPress={savePhotos}>
          <Text style={styles.saveButtonText}>Salvar Fotos de doc</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.noPhotosButton} onPress={goToMap}>
          <Text style={styles.noPhotosButtonText}>Nao tenho fotos</Text>
        </TouchableOpacity>
      </View>

      <Text style={styles.title}>Tirar fotos do documento</Text>
    </View>
  );
}
